...
I want us to think about code in the large and in the small, and along the way we'll be addressing the key questions of tabs vs spaces, JavaScript vs TypeScript, Norwich vs Ipswich, and whether Thor is stronger than the Hulk.

No, of course, we won't, because those things are insoluble and, whisper it, unimportant. There are so many insoluble important things we could be thinking about instead.
...
So if I did say to you "write me some code", what would you immediately ask of me?
--
...
[NOTE.speaker]
--
When we write code, we're taking stuff that we just think up - it comes from inside us  - and communicate that as directly as we can into the computer.  And, after a little of bit of practice, that's an easy process.
...
[NOTE.speaker]
--
I've talked so far here about us. What _we_ can imagine, what _we_ can express, what each of us, individually and together, can imagine and express.
...
We can write reams and reams of it at almost no cost. We can do incredible computations in fractions of a second, we chew through vast quantities of data in moments. I'm not talking about things that would astonish our grandparents, I'm taking about capabilities that would amaze ourselves of 10 years ago, five years ago even.

We can do these incredible things, just like that. It's like we have magic in our fingertips. This is ours to command. But the ease with which we can produce that code, can crunch those numbers, that open up these huge possibilities, these vast multidimensional hyperspace manifolds we can explore - which is why it's so thrilling - is also why writing code can be so very, very hard. How do we navigate this myriad of possibilities?
...
I've already suggested that we have these different forces in play, and that we're going to be engaged in some kind of a balancing act between, well not good and evil exactly, or even between law and chaos - would that it were so binary - we're trying to find a compromise between the ease with which we can express our idea, the difficulty we sometimes have in expressing it, what it is we actually need to say, what it is that people want us to say.


but between everyone with an interest in the code <<<< this doesn't follow from what I've said so far

Consequently, there's a tension between these guidelines, and actually within the guidelines themselves. And because they're not rules, there's no single right way to apply them. We have to find out, keep on finding out, what works for each of us, in our context.
